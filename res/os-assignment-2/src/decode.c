...
{
    if (argc != 2 && argc != 3) {
        printf("decode <library>\n");
        printf("decode <library> <text>\n");
    ...
    decode_function decodeFunc = dlsym(handle, "decode");

    if (!handle) {
		printf("Cannot load the function decode!\n");
		return 1;
	}
